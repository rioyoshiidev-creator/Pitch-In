...
async function fireAlarms(matchId: string, matchDate: string, starterIds: Set<string>) {
  const { data: alarms } = await supabase
    .from('alarms')
    .select('id, alarm_timing, minutes_before, snooze, selected_player_ids, devices(push_token)')
    .eq('match_id', matchId)
    .eq('is_enabled', true)
  ...
  for (const alarm of alarms ?? []) {
    ...
    // 選手選択が設定されている場合、選択中の選手にスタメンがいるかチェック
    if (selectedIds && !selectedIds.some((id) => starterIds.has(id))) continue
